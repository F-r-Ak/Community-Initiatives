...
export class DevelopmentEntitiesService extends HttpService {
    protected get baseUrl(): string {
        return 'v1/developmententity/';
    }

    getDevelopmentEntity(id: string) {
        return this.get<Lookup>({ apiName: `Get/${id}` });
    }

    getEditDevelopmentEntity(id: string) {
        return this.get<Lookup>({ apiName: `getEdit/${id}` });
    }

    get developmentEntities() {
