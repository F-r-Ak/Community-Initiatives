...
export abstract class BaseListComponent extends BaseComponent implements OnInit {
    ...
    openDialog(component: any, pageTitle: any, data: any, options: boolean | { closable?: boolean; dismissableMask?: boolean } = true): void {
        // Support both legacy boolean `closable` and new `options` object for extensibility
        const normalized = typeof options === 'boolean' ? { closable: options, dismissableMask: false } : { closable: options.closable ?? true, dismissableMask: options.dismissableMask ?? false };
        ...
        this.dialogRef = this.dialogService.open(component, {
            ...
            autoZIndex: true,
            baseZIndex: 10000,
            // Prevent closing when clicking outside by default (dismissableMask: false)
            dismissableMask: normalized.dismissableMask,
            closable: normalized.closable // Set the closable property
        });
    ...
    loadDataFromServer(): void {
        this.dataTableService.loadData(this.tableOptions.inputUrl.getAll).subscribe({
            ...
            error: (err) => {
                if (err.status === 401) {
                this.alert.error(this.localize.translate.instant('ERRORS.401'));
    ...
    resetOpt(): void {
        ...
        const tableFilter = this.tableOptions?.bodyOptions?.filter;
        this.dataTableService.opt.filter = tableFilter !== null && tableFilter !== undefined ? tableFilter : this.dataTableService.opt.filter;

        // Many endpoints expect `appId` to always exist; default safely to 0 when not provided.
        const appId = this.tableOptions?.appId ?? 0;
        if (this.tableOptions?.includeAppId === false) {
            // Respect explicit opt-out
            delete (this.dataTableService.opt.filter as any).appId;
        } else {
            (this.dataTableService.opt.filter as any).appId = appId;
        }
    }
