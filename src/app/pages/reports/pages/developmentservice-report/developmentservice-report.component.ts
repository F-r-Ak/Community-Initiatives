...
import { FieldsService,DevelopmentServiceService,TeamMembersService, CitiesService, PrimeTitleToolBarComponent, PrimeAutoCompleteComponent, PrimeInputTextComponent ,PrimeDatepickerComponent,GendersService} from './../../../../shared';
import { ButtonModule } from 'primeng/button';
import { AuthHelper , AlertService,} from './../../../../core';
import { take } from 'rxjs/operators';
@Component({
    selector: 'app-developmentservice-report',
  ...
  templateUrl: './developmentservice-report.component.html',
  styleUrl: './developmentservice-report.component.scss'
})
export class DevelopmentServiceReportComponent implements OnInit {
    form!: FormGroup;
    formBuilder = inject(FormBuilder);
    fieldService = inject(FieldsService);
    gendersService = inject(GendersService);
   developmentService = inject(DevelopmentServiceService);
    ...
    initFormGroup() {
        ...
        //    fieldName :[''],
           cityId: [''],
            ...
            reportName: ['DevelopServiceReport', [Validators.required, Validators.minLength(1)]],
   ...
   searchName(event: any) {
        ...
        this.developmentService.developmentServices.pipe(take(1)).subscribe((res: any[]) => {
...
onReportNameSelect(event: any) {
 ...
 searchReportNames(event: any) {
        const query = event.query.toLowerCase();
        this.reportNames = [
            { label: 'خدمات التنموية', value: 'DevelopServiceReport' }
    ...
    onExecute() {
        ...
        if (this.form.valid) {
           ...
           this.developmentService.downloadReport(dataToClean).subscribe({
    next: (blob: Blob) => {
