...
import { SubmitButtonsComponent, PrimeInputTextComponent, TeamMembersService } from '../../../../../shared';
...
export class AddEditTeamMemberComponent extends BaseEditComponent implements OnInit {
    ...
    submit() {
        if (this.pageType === 'add')
            this.teamMembersService.add(this.form.value).subscribe(() => {
                this.closeDialog();
            });
        if (this.pageType === 'edit')
            this.teamMembersService.update({ id: this.id, ...this.form.value }).subscribe(() => {
                this.closeDialog();
            });
    }
